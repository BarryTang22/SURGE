...
#
# Modified for SURGE (Surprise-Guided Token Reduction) integration.
...
class Qwen2_5_VLModel(Qwen2_5_VLPreTrainedModel):
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        _video_token_mask = (input_ids == self.config.video_token_id) if input_ids is not None else None
        ...
        surge_keep_masks = None
        if pixel_values_videos is not None:
            video_embeds = self.get_video_features(pixel_values_videos, video_grid_thw)

            if self.surge_config is not None and (past_key_values is None or past_key_values.get_seq_length() == 0):
                from .surge_core import compute_surge
                spatial_merge_size = self.config.vision_config.spatial_merge_size
                surge_keep_list = []
                for vid_idx, vid_embed in enumerate(video_embeds):
                    T_i = video_grid_thw[vid_idx][0].item()
                    H_i = video_grid_thw[vid_idx][1].item() // spatial_merge_size
                    W_i = video_grid_thw[vid_idx][2].item() // spatial_merge_size
                    surge_out = compute_surge(vid_embed, T_i, H_i, W_i, self.surge_config)
                    surge_keep_list.append(surge_out.keep_mask)
                surge_keep_masks = torch.cat(surge_keep_list, dim=0)
        ...
        if surge_keep_masks is not None:
            from .surge_core import apply_surge_to_sequence

            video_token_mask = _video_token_mask

            inputs_embeds, position_ids, attention_mask, cache_position = apply_surge_to_sequence(
                inputs_embeds=inputs_embeds,
                position_ids=position_ids,
                attention_mask=attention_mask,
                video_token_mask=video_token_mask,
                surge_keep=surge_keep_masks,
                cache_position=cache_position,
            )
...
class Qwen2_5_VLForConditionalGeneration(Qwen2_5_VLPreTrainedModel, GenerationMixin):
    ...
    def set_surge_config(self, surge_config):
        """Enable SURGE token pruning with the given SurgeConfig."""
        self.model.surge_config = surge_config

    def disable_surge(self):
        """Disable SURGE token pruning."""
        self.model.surge_config = None

    def set_surge_star_config(self, surge_star_config):
        """Enable SURGE-star (CLIP-based event focusing). Lazy-loads CLIP model."""
        self._surge_star_config = surge_star_config
        # Lazy-load CLIP model
        import clip as clip_module
        self._clip_model, self._clip_preprocess = clip_module.load(
            surge_star_config.clip_model_name, device=next(self.parameters()).device
        )
